...
static void parse_args(int argc, char* argv[])
{
    ...
    static struct option long_options[] = {
        ...
        /* iacobucci's options */
        { "new-coordinates", required_argument, 0, 0 },
...
#define OPT_NAME_IS(name) (strcmp(opt_name, (name)) == 0)
        ...
        case 0:
            if (OPT_NAME_IS("display")) {
                ...
                break;
            } else if (OPT_NAME_IS("new-coordinates")) {
                config.move_cursor = true;

                char* delim = ",";

                config.new_coordinates.x = parse_int(strsep(&optarg, delim));
                config.new_coordinates.y = parse_int(strsep(&optarg, delim));
...
static void print_usage(char* argv[])
{
    fprintf(stderr, "Usage: %s [--new-coordinates <x,y>] [--timeout <n>] [--jitter <radius>] [--exclude-root] [--ignore-scrolling] [--ignore-buttons <buttons>] [--hide-on-touch] [-b|--fork] [-v|--version] [-h|--help] [--start-hidden]", argv[0]);
    fprintf(stderr, "\n");
    exit(EXIT_FAILURE);
}
